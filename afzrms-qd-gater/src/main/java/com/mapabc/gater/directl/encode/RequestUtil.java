		...
		for (TStructions bean : insList) {
			...
			try {
				...
				if (mttype != null) {
					...
					try {
						String sendCount = "0";
						if(bean.getSendCount()!=null || bean.getSendCount() !=""){
							sendCount = String.valueOf(Integer.parseInt(bean.getSendCount()) + 1);
						}
						dbservice.updateInstructions(bean.getId(), ins, bean
								.getType(), null, new java.sql.Date(System
								.currentTimeMillis()),sendCount , String.valueOf(result));
						if (log.isInfoEnabled())
