...
public class TjTermAlarmServiceImpl implements TjTermAlarmService {
	...
	@Override
	public PageVo<TjTermAlarmVo> findListToday(Integer pageNo, Integer pageSize,  String name,String deptName)
			throws Exception {
		String sql = "select tj.device_id,tj.tj_date,tj.speedAlarm,tj.areaAlarm,tj.areaSpeedAlarm,t.name,dept.name deptName,"
				+ " type.`name` vetypeName,brand.name brandName"
				+ " from (select l.device_id,STR_TO_DATE(now(),'%Y-%m-%d')  tj_date,"
				+ "	count(case when l.alarm_type=1 then 1 end ) speedAlarm,"
				+ "	count(case when l.alarm_type=2 then 1 end) areaAlarm,"
				+ "	count(case when l.alarm_type=3 then 1 end) areaSpeedAlarm"
				+ "	from T_LOCRECORD l "
				+ "	where 1=1 "
				+ "	and l.gps_time >= STR_TO_DATE(CURDATE(),'%Y-%m-%d %T')"
				+ "	and l.gps_time <= STR_TO_DATE(CURRENT_TIMESTAMP(),'%Y-%m-%d %T')"
				+ "	and l.distance<=100 group by l.device_id) tj"
				+ " left join T_TERMINAL t on tj.device_id=t.device_id"
				+ " left join T_VEHICLE_TYPE type on type.id = t.vehicle_type_id"
				+ " left join T_VEHICLE_BRAND brand on brand.id = t.vehicle_brand_id "
				+ " left join T_DEPT dept on t.dept_id = dept.id";
		...
		for (Object[] objs : pagingQueryWithSql.getResult()) {
			String _deviceId = (String) objs[0];
			Date _tjDate = (Date) objs[1];
			BigInteger _speedAlarmCount = (BigInteger) objs[2];
			BigInteger _areaAlarmCount = (BigInteger) objs[3];
			BigInteger _areaSpeedAlarmCount = (BigInteger) objs[4];
			...
			TjTermAlarmVo vo = new TjTermAlarmVo(_deviceId, _tjDate, _speedAlarmCount.intValue(), _areaAlarmCount.intValue(),
					_areaSpeedAlarmCount.intValue(), _name,deName,vetypeName,vehicleBrand);
			data.add(vo);
		}
		return new PageVo<TjTermAlarmVo>(pagingQueryWithSql.getTotalCount(), data);
	}
