...
public class VehicleTypeServiceImpl implements VehicleTypeService {
	...
	@Override
	public Integer add(String name, String uses, String remark, String createBy,String color) {
		TVehicleType tVehicleType = new TVehicleType(name, uses, createBy, new Date(), remark, color,null);
		tVehicleTypeDao.save(tVehicleType);
		return tVehicleType.getId();
	}

	@Override
	public Integer update(Integer id, String name, String description, String remark,String color) {
		TVehicleType findById = tVehicleTypeDao.findById(TVehicleType.class, id);
		if (findById != null) {
			findById.setName(name);
			findById.setColor(color);
	...
	private VehicleTypeVo helpPo2Vo(TVehicleType tVehicleType) {
		...
		VehicleTypeVo vehicleTypeVo = new VehicleTypeVo(tVehicleType.getId(), tVehicleType.getName(),
				...
				tVehicleType.getRemark(),tVehicleType.getColor());
