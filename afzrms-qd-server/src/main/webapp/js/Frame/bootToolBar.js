	...
	GAS.StatusBar = Ext.extend(Ext.Toolbar, {
					    ...
					    afterrender:function(panel){
					       ...
					       container.on({
					           ...
					           mousedown :function(e,t){
					                var flg = Ext.fly(t);
					                if(flg && flg.hasClass(this.settingClass)){
		...
		getCopyright : function(){
			if(!this.copyrightText){
				this.copyrightText = "<span style='margin-left:4px;'>"+this.copyRight+"</span>";
			}
		...
		getSettingAlarm : function(){
			if(!this.setMsgWindowImg){
				var title = "报警弹窗设置";
				this.setMsgWindowImg = "<img style='cursor:pointer;margin-left:4px;margin-right:1px;' class='"+this.settingClass+"' alt='"+title+"' title='"+title+"' src=" + this.settingIcon+">";
			}
