(function(){
	...
	GAS.VerchileTypManager = Ext.extend(Ext.Window,{
		     constructor : function(config) {
		     	...
		     	this.updateUrl    = GAS.config["globalProxy"]+"/vehicleType/update?_dc="+new Date().getTime(); //修改的URL地址
				this.addUrl       = GAS.config["globalProxy"]+"/vehicleType/add?_dc="+new Date().getTime(); //添加的URL地址
		   ...
		   loadForm : function(){
				if (this.action == "update"){
					 function loadrecord(){
					    ...
					    this.colorFiled.setValue(result["color"] || "000000");
					 }
					 this.on({
					    scope : this,
					    show  : function(){
		   ...
		   getFormPanel:function(){
				if(!this.formPanel){
					  ...
					  this.nameFiled = new Ext.form.TextField({
					   ...
					   //名称
					  this.colorFiled = new  Ext.form.ColorField({
					      fieldLabel: "种类颜色",
					      name      : "name",
					      allowBlank: false,
					      width     :300,
