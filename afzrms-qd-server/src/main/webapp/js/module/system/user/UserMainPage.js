(function(){
	...
	GAS.UserQueryMainPage = Ext.extend(Ext.Panel,{
	      constructor : function(config) {
	      	    ...
	      	    this.pageSize = 20;
	      	    this.rootNodeId  = 1;
	      	    this.userListUrl = GAS.config["globalProxy"]+"/user/list?_dc="+new Date().getTime(); //查询接口
	      	    this.deleteUrl   =  GAS.config["globalProxy"]+"/user/delete?_dc="+new Date().getTime(); //删除接口
		...
		getGridStore : function() {
		     if(!this.gridstore){
		     	 this.gridstore= new Ext.data.JsonStore({
		     		autoLoad : true,
				 	fields   : ["id","deptId","groupIds","deptName","remark","name","isUsed","account","userType","roleId","createTime","contact","createBy","roleName"],
				 	root     : 'data', 
					totalProperty: 'total', 
		...
		getGridColumn:function(){
		    var cls =  [
			   ...
			   this.getGridLineNum(),
			   {
			     ...
			     flex :1,
			     renderer:function(v){
			     	return v==0?"<span style='color:red;'>系统用户</span>":"一般用户";
			     }
			   },{
			     header:"创建人",
			     sortable:true,
