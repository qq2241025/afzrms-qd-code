(function() {
	 ...
	 * QQ:1023553356
	 */
	GAS.DeptViolatingStat = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			config = config || {};
			this.pageSize = 20;
			this.deptStatUrl = GAS.config["globalProxy"] + "/tj/deptAlarm?_dc="+new Date().getTime(); 
			this.exportUrl = GAS.config["globalProxy"] + "/tj/deptAlarmExport?_dc="+new Date().getTime(); 
			Ext.apply(this, {
				border : false,
				frame : false,
				layout : "fit",
			...
			GAS.DeptViolatingStat.superclass.constructor.apply(this, arguments);
		},
		// 主面板
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
					autoLoad : false,
					fields : ["tjDate", "areaAlarmCount", "speedAlarmCount", "areaSpeedAlarmCount", "name","deptId"],
					root : 'data',
					totalProperty : 'total',
					proxy : new Ext.data.HttpProxy({
						url : this.deptStatUrl
					}),
					listeners : {
						scope : this,
		...
		getGridToolBar : function() {
			if (!this.gridtoolBar) {
				...
				//部门
				this.dept = new Ext.form.TextField({
					fieldLabel : "车辆牌号",
					name : "dept",
					width : 100,
					allowBlank : false,
		...
		getQueryParams : function(){
		   ...
		   params["name"] = this.dept.getValue();
		   return params;
		},
		//报警查询
