(function(){
	...
	GAS.AreaBindMainPage = Ext.extend(Ext.Panel,{
	      constructor : function(config) {
		     	...
		     	this.ruleListUrl       = GAS.config["globalProxy"]+"/alarmruleArea/list?_dc="+new Date().getTime(); //规则
		     	this.deleteUrl = GAS.config["globalProxy"]+"/alarmruleArea/delete?_dc="+new Date().getTime(); //规则
		     	this.bindAreaAlarm    = GAS.config["globalProxy"]+"/terminalInstruction/areaAlarm?_dc="+new Date().getTime(); //绑定
		     	this.unbindAreaAlarm  = GAS.config["globalProxy"]+"/terminalInstruction/cancleArea?_dc="+new Date().getTime(); //取消绑定
		     	
		     	this.ruleManagerUrl = ["js/module/system/areaBind/AreaBindManager.js","js/module/system/areaBind/RowEditor.js"]; //加载的URL地址
		...
		getGridStore : function() {
		     if(!this.gridstore){
		     	 this.gridstore= new Ext.data.JsonStore({
		     		autoLoad : true,
				 	fields   : ["id","name","remark","name","createTime","createBy","areas"],
				 	root     : 'data', 
					totalProperty: 'total', 
		...
		setBindTreeNodeStyle:function(list){
		     if(list && list.length > 0){
		         for (var i = 0;  i< list.length; i++) {
		         	  var targetId= list[i];
		         	  this.getLeftPanel().setNodeTextColor(targetId,"blue");
		         }
		     }
		},
		//添加修改公共方法
		...
		bindDevices:function(){
		   var list = this.getSelectTreeDevideList(),me = this;
		   if (list.length>0) {
				 var rulelist = this.getGridPanel().getSelectionModel().getSelections();
