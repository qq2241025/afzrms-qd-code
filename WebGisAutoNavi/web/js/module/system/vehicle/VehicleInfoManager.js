(function(){
	...
	GAS.VehicleInfoManager = Ext.extend(Ext.Window,{
		     constructor : function(config) {
		     	...
		     	this.updateUrl    = GAS.config["globalProxy"]+"/terminal/update?_dc="+new Date().getTime(); //修改的URL地址
				this.addUrl       = GAS.config["globalProxy"]+"/terminal/add?_dc="+new Date().getTime(); //添加的URL地址
		     	this.comboxTreeUrl= GAS.config["globalProxy"]+"/dept/tree?_dc="+new Date().getTime(); //树形菜单
		     	this.verTypeUrl   = GAS.config["globalProxy"]+"/vehicleType/list?_dc="+new Date().getTime(); //车辆类型
				this.verBrandUrl   = GAS.config["globalProxy"]+"/vehicleBrand/list?_dc="+new Date().getTime(); //车辆品牌
		     	this.protocalUrl  = GAS.config["globalProxy"]+"/protocalType/list?_dc="+new Date().getTime(); //协议类型
					 ...
					 function loadrecord(){
					    var result = this.loadRecord;
						 ...
						 this.remarker.setValue(result["remark"]);
						  ...
						  // <8>
						 this.driverName.setValue(result["driverName"]);
						  // <8>
						 this.driverPhone.setValue(result["driverPhone"]);
					 }
					 this.on({
					    scope : this,
					    show  : function(){
		   ...
		   getFormPanel:function(){
				if(!this.formPanel){
					  ...
					  this.driverName = new Ext.form.TextField({
					   ...
					   //驾驶员姓名
					  this.driverPhone = new Ext.form.TextField({
					      fieldLabel: "司机手机号码",
					      name : "diverName",
					      allowBlank:false,
					      anchor : "92%"
		   ...
		   //关闭
			...
			getSubmitParam : function(){
			    ...
			    params["driverName"] = this.driverName.getValue();
			    params["driverPhone"] = this.driverPhone.getValue();
