(function(){
	...
	GAS.RoleManager = Ext.extend(Ext.Window,{
		     constructor : function(config) {
				...
				this.updateUrl       = GAS.config["globalProxy"]+"/role/update?_dc="+new Date().getTime(); //修改的URL地址
				this.addUrl          = GAS.config["globalProxy"]+"/role/add?_dc="+new Date().getTime(); //添加的URL地址
				this.comboxTreeUrl   = GAS.config["globalProxy"]+"/dept/tree?_dc="+new Date().getTime(); //部门下拉列表
				this.moduleTreeUrl   = GAS.config["globalProxy"]+"/module/tree?_dc="+new Date().getTime(); //模块菜单
						    ...
						    var setNodeValue=function(){
						        if(moduleList && moduleList.length > 0){
							        Ext.each(moduleList,function(vale){
							        	 ...
							        	 var record = this.getModuleTree().findNodeByTargetId(intNodeId);
				...
				if(!this.formPanel){
					 ...
					 this.roleType =new Ext.form.ComboBox({
					    ...
					    store : new Ext.data.JsonStore({
						 	fields   : ["value","text"],
						 	root     : 'data', 
							data     : {data:[{value:  true,text : "系统管理员"},{ value:  false,text : "普通管理员"}]}
				     	 }),
				     	value : true,
					    valueField: 'value',
					    displayField: 'text'
			    ...
			    var callback = function(res,req){
		    	     var result = Ext.decode(res.responseText);
				 	 if(result["success"] && result["success"]=="true"){
				 		...
				 		Ext.Msg.alert("信息提示", result["result"] || result["msg"] || "提交成功");
				 		this.windowClose();
				     }else{
				     	Ext.Msg.alert("信息提示",  result["result"] || result["msg"] || "提交失败");
				     }
			    };
			    GAS.AOP.send({
