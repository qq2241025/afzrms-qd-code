(function(){
	...
	GAS.VehicleGroupManager = Ext.extend(Ext.Window,{
		     constructor : function(config) {
		     	...
		     	this.updateUrl    = GAS.config["globalProxy"]+"/vehicleGroup/update?_dc="+new Date().getTime(); //查询接口
				this.addUrl       = GAS.config["globalProxy"]+"/vehicleGroup/add?_dc="+new Date().getTime(); //删除接口
				
				this.termTreeUrl  = GAS.config["globalProxy"]+"/dept/treeTerminal?_dc="+new Date().getTime(); //终端树形列表
				...
				GAS.VehicleGroupManager.superclass.constructor.apply(this, arguments);
		   },
		   // settting title 
						    ...
						    var setNodeValue=function(){
						        if(vehicleList && vehicleList.length > 0){
							        Ext.each(vehicleList,function(vale){
							        	 ...
							        	 var record = this.getVehicleTreePanel().findNodeByTargetId(intNodeId);
