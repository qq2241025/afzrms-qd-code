(function(){
	...
	GAS.DeptManager = Ext.extend(Ext.Window,{
		     constructor : function(config) {
		     	...
		     	this.updateUrl     = GAS.config["globalProxy"]+"/dept/update?_dc="+new Date().getTime(); //修改的URL地址
				this.addUrl        = GAS.config["globalProxy"]+"/dept/add?_dc="+new Date().getTime(); //添加的URL地址
		   ...
		   loadForm : function(){
				if (this.action == "update"){
					     ...
					     var id = record["id"],parentId = record["parentId"],
					     duty=record["duty"],remark=record["remark"],
					     name=record["name"],director=record["director"],
					     sortNum=record["sortNum"],parentName=record["parentName"];
