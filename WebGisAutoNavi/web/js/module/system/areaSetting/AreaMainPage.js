(function(){
	...
	GAS.AreaAlarmMainPage = Ext.extend(Ext.Panel,{
	      constructor : function(config) {
	      	    ...
	      	    this.deleteUrl   = GAS.config["globalProxy"]+ "/alarmArea/delete?_dc="+new Date().getTime();
		...
		getGridStore : function() {
		     if(!this.gridstore){
		     	 this.gridstore= new Ext.data.JsonStore({
		     		autoLoad : true,
				 	fields   : ["id","isUsed","createTime","xys","description","name","createBy","remark"],
				 	root     : 'data', 
					totalProperty: 'total', 
			 ...
			 GAS.AOP.send({
				...
				callbackFn : function(res) {
					...
					if (res["success"]) {
						...
						GAS.cacheAlarmCache.deleteAlarmCacheIds(list); //同步缓存
