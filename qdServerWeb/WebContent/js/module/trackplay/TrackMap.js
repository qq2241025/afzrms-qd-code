(function() {
	...
	GAS.TrackQueryMap = Ext.extend(GAS.BaseMap, {
		constructor : function(config) {
			...
			this.maxLimit = 60000;
			// 轨迹列表Url
			// this.trackDataUrl = "data.json?_dc="+new Date().getTime();
			this.trackDataUrl = GAS.config["globalProxy"]
					+ "/locrecord/list?_dc=" + new Date().getTime();
		...
		playTrack : function() {
			...
			GAS.asyncEach(dataArray, function(data, dataindex) {
					...
					}, 1, this, this.commboSpeed.getValue(), this
							.getPalyIndex());
		},
		// 轨迹数据中间处理逻辑
		proccessTrackData : function(list, data, index) {
			...
			var dirtion = data["direction"] || 0;
			var dirAndIcon = GAS.getGPSDirection(dirtion);
			var dirIcon = GAS.config["gpsimagePath"] + dirAndIcon["key"]
					+ ".png";; // 方向图标
			if (index == 0) {
				var cfg = {
					lnglat : currentPoint,
				...
				this.updateMarkPostion(this.pointTrackIcon, currentPoint); // 更新点的位置信息
			}
			if (playType == "1") { // 追踪模式
		...
		playTrackBar : function() {
			...
			this.bottombar = new Ext.Toolbar({
						disabled : true,
						items : [this.startButton, "-", this.parseButton, "-",
								"播放模式", this.playType, "播放速度",
								this.commboSpeed, "		", this.showPointsCheckbox]
					});
		...
		showTrackDataOnMap : function(trackData) {
			...
			if (trackData && trackData.length > 0) {
				...
				for (var i = 0; i < trackData.length; i++) {
					...
					var point = this.createPoint(parseFloat(lng),
							parseFloat(lat));
					var contentText = [
							'<b>车牌号码: </b> ' + termName,
							// '<br><b>手机号码: </b> ' + simcard,
		...
		showTrackPointIcon : function() {
			...
			var trackData = this.TrackPlayData, startRecord = trackData[0], endRecord = trackData[trackData.length
					- 1];
			// 开始点
			var firstCfg = {
				...
				clickEnable : true,
				contentMsg : startRecord["contentMsg"]
			};
			// 结束点
			var endCfg = {
				...
				clickEnable : true,
				contentMsg : endRecord["contentMsg"]
			};
		...
		showMidTrackMarker : function() {
			...
			var count = this.setIntervalCount, time = this.setIntervalTime, list = this.TrackPlayData, // 轨迹数据
			len = list.length;
			...
			this.myTask = this.doTimerTask(function(aa) {
						...
						for (var index = startNum; index < endNum; index++) {
							...
							if (dataIndex == 0 || dataIndex == len - 1) {
								...
								this.TrackPlayData_marker.push(this
										.showTrackPoint(midCfg));
							}
							dataIndex++;
						}
						pagestart++;
