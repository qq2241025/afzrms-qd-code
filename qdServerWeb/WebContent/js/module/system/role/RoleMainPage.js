(function() {
	...
	GAS.RoleMainPage = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			this.pageSize = 20;
			this.rootNodeId = 1;
			this.roleListUrl = GAS.config["globalProxy"] + "/role/list?_dc="
					+ new Date().getTime(); // 查询接口
			this.deleteUrl = GAS.config["globalProxy"] + "/role/delete?_dc="
					+ new Date().getTime(); // 删除接口
			this.selectRoleModUrl = GAS.config["globalProxy"]
					+ "/role/moduleList?_dc=" + new Date().getTime(); // 查询角色对象的模块列表
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["id", "deptName", "createTime", "remark",
		...
		getGridToolBar : function() {
			if (!this.gridtoolBar) {
				...
				this.gridtoolBar = new Ext.Toolbar({
							...
							items : ["角色名称:", this.roleName, "所属部门",
									this.upDept, querybtn, "->"].concat(this
									.getToorBtn())
						});
			}
