	...
	GAS.VehicleInfoManager = Ext.extend(Ext.Window, {
		constructor : function(config) {
			...
			this.updateUrl = GAS.config["globalProxy"]
					+ "/terminal/update?_dc=" + new Date().getTime(); // 修改的URL地址
			this.addUrl = GAS.config["globalProxy"] + "/terminal/add?_dc="
					+ new Date().getTime(); // 添加的URL地址
			this.comboxTreeUrl = GAS.config["globalProxy"] + "/dept/tree?_dc="
					+ new Date().getTime(); // 树形菜单
			this.verTypeUrl = GAS.config["globalProxy"]
					+ "/vehicleType/list?_dc=" + new Date().getTime(); // 车辆类型
			this.verBrandUrl = GAS.config["globalProxy"]
					+ "/vehicleBrand/list?_dc=" + new Date().getTime(); // 车辆品牌
			this.protocalUrl = GAS.config["globalProxy"]
					+ "/protocalType/list?_dc=" + new Date().getTime(); // 协议类型
			...
			GAS.VehicleInfoManager.superclass.constructor
		...
		loadForm : function() {
			if (this.action == "update") {
				function loadrecord() {
					...
					console.log(result);
					// <1>
					this.upDept.setHiddenValue(result["deptId"],
							result["deptName"]); // 上级部门
					// <2>
					...
					this.vehicleBrand.getStore().on({
						...
						load : function(store) {
							this.vehicleBrand
									.setValue(result["vehicleBrandId"]);
						}
					})
					// <4>
		...
		getFormPanel : function() {
			if (!this.formPanel) {
				...
				var list = [this.deviceId, this.vehicleNumber,
						this.vehicleType, this.vehicleBrand, this.simcard,
						this.locateType, this.protocalType, this.upDept,
						this.remarker];
