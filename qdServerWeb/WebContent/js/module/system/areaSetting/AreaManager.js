	...
	GAS.AreaManager = Ext.extend(Ext.Window, {
				constructor : function(config) {
					...
					this.updateUrl = GAS.config["globalProxy"]
							+ "/alarmArea/update?_dc=" + new Date().getTime();
					this.addUrl = GAS.config["globalProxy"]
							+ "/alarmArea/add?_dc=" + new Date().getTime();
					...
					GAS.AreaManager.superclass.constructor.apply(this,
							arguments);
				},
				// settting title
				...
				getFormPanel : function() {
					if (!this.formPanel) {
						...
						var list = [this.formAreaname, this.descript,
								this.areaId, this.remark];
				...
				// 获取经纬度坐标列表
				getMapXys : function() {
					var points = this.getCenterPanel()
							.getDrawPolugonLatLngList();
