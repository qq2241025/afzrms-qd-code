(function() {
	...
	GAS.PermissionUserQuery = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			GAS.PermissionUserQuery.superclass.constructor.apply(this,
					arguments);
		},
		// 主面板
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["username", "deptName", "phone", "email",
									"worker", "endserverTime", "createTime",
									"desc", "id"],
							root : 'data',
							totalProperty : 'total',
		...
		getGridToolBar : function() {
			if (!this.gridtoolBar) {
				...
				this.gridtoolBar = new Ext.Toolbar({
							...
							items : ["用户名称:", this.userName, querybtn, "->"]
