(function() {
	...
	GAS.AreaBindSetting = Ext.extend(Ext.Window, {
		constructor : function(config) {
			...
			this.addAreaRuleUrl = GAS.config["globalProxy"]
					+ "/alarmruleArea/add?_dc=" + new Date().getTime();
			this.updateAreaRuleUrl = GAS.config["globalProxy"]
					+ "/alarmruleArea/update?_dc=" + new Date().getTime();
		...
		getFormPanel : function() {
			if (!this.formPanel) {
				...
				var list = [this.ruleName, this.ruleId, this.remark];
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["id", "isUsed", "areaType", "speed",
									"createTime", "xys", "description", "name",
									"createBy", "remark"],
							root : 'data',
							totalProperty : 'total',
				...
				this.editor = new Ext.ux.grid.RowEditor({
					...
					listeners : {
						...
						showEditor : function(roweditor, rowIndex, bts) {
							var record = this.getGridStore().getAt(rowIndex), areaType = record
									.get("areaType");
		...
		getGridColumn : function() {
			...
			var cls = new Ext.grid.ColumnModel({
				columns : [this.getCheckboxModel(), this.getGridLineNum(), {
							header : "id",
							hidden : true,
							dataIndex : "id",
							flex : 1
							...
							renderer : function(val, cls, record) {
								...
								var newValue = val == 0
										? "<b style='color:blue'>进区域</b>"
										: val == 1
												? "<b style='color:red'>出区域</b>"
												: "<b style='color:green'>进出区域限速</b>";
								return newValue;
							}
						}, {
							header : "设置速度",
		...
		getAreas : function() {
			...
			if (list && list.length > 0) {
				var index = 0;
				for (var i = 0; i < list.length; i++) {
					var dt = list[i], id = dt.get("id"), areaType = dt
							.get("areaType"), speed = dt.get("speed");
		...
		onSubmit : function(params) {
			var callback = function(res, req) {
				...
				if (result["success"] && result["success"] == "true") {
					...
					Ext.Msg.alert("信息提示", result["msg"] || result["result"]
									|| "添加成功");
					this.windowClose();
				} else {
					Ext.Msg.alert("信息提示", result["msg"] || result["result"]
