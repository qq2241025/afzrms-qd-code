(function() {
	...
	GAS.VechileBrand = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			this.vehicleListUrl = GAS.config["globalProxy"]
					+ "/vehicleBrand/list?_dc=" + new Date().getTime(); // 查询接口
			this.deleteUrl = GAS.config["globalProxy"]
					+ "/vehicleBrand/delete?_dc=" + new Date().getTime(); // 删除接口
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["id", "createTime", "remark",
									"description", "name", "createBy"],
							root : 'data',
							totalProperty : 'total',
		...
		getGridToolBar : function() {
			if (!this.gridtoolBar) {
				...
				this.gridtoolBar = new Ext.Toolbar({
							...
							items : ["车辆品牌:", this.typename, querybtn, "->"]
