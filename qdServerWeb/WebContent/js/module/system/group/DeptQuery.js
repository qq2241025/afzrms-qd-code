(function() {
	...
	GAS.DeptQuery = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			this.deptListUrl = GAS.config["globalProxy"] + "/dept/list?_dc="
					+ new Date().getTime(); // 查询接口
			this.deleteUrl = GAS.config["globalProxy"] + "/dept/delete?_dc="
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["id", "parentId", "duty", "remark",
									"name", "director", "sortNum", "parentName"],
							root : 'data',
							totalProperty : 'total',
		...
		getGridToolBar : function() {
			if (!this.gridtoolBar) {
				...
				this.gridtoolBar = new Ext.Toolbar({
							...
							items : ["部门名称:", this.deptname, querybtn, "->"]
