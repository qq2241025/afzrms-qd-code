(function() {
	...
	GAS.DeptViolatingStat = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			config = config || {};
			this.pageSize = 20;
			this.deptStatUrl = GAS.config["globalProxy"] + "/tj/deptAlarm?_dc="
					+ new Date().getTime();
			Ext.apply(this, {
						border : false,
						frame : false,
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["areaAlarmCount", "speedAlarmCount",
									"areaSpeedAlarmCount", "name", "deptId"],
							root : 'data',
							totalProperty : 'total',
			...
			if (!this.gridtoolBar) {
				...
				this.gridtoolBar = new Ext.Toolbar({
							...
							items : ["开始时间:", this.alarmStart, "结束时间:",
									this.alarmEnd, "部门:", this.dept, querybtn]
						});
			}
