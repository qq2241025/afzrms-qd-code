(function() {
	...
	GAS.VehicleLineStatusStat = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			config = config || {};
			this.pageSize = 20;
			this.onlineStatUrl = GAS.config["globalProxy"]
					+ "/tj/termOnoffline?_dc=" + new Date().getTime();
			Ext.apply(this, {
						border : false,
						frame : false,
						layout : "fit",
			...
			GAS.VehicleLineStatusStat.superclass.constructor.apply(this,
					arguments);
		},
		// 主面板
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["offlineCount", "tjDate", "onlineRate",
									"onlineCount"],
							root : 'data',
							totalProperty : 'total',
