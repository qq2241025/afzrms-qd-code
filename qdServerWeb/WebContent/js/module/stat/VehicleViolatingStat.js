(function() {
	...
	GAS.VehicleViolatingStat = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			config = config || {};
			this.pageSize = 20;
			this.VehicleStatUrl = GAS.config["globalProxy"]
					+ "/tj/termAlarm?_dc=" + new Date().getTime();
			Ext.apply(this, {
						border : false,
						frame : false,
						layout : "fit",
			...
			GAS.VehicleViolatingStat.superclass.constructor.apply(this,
					arguments);
		},
		// 主面板
		...
		getGridStore : function() {
			if (!this.gridstore) {
				this.gridstore = new Ext.data.JsonStore({
							autoLoad : true,
							fields : ["deviceId", "areaAlarmCount",
									"speedAlarmCount", "tjDate",
									"areaSpeedAlarmCount", "name"],
							root : 'data',
							totalProperty : 'total',
