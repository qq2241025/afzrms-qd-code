(function() {
	...
	GAS.MonitorVehicle = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			this.addEvents("mintorClick", "alarmClick");
		...
		getMintorStore : function() {
			if (!this.mintorStore) {
				this.mintorStore = new Ext.data.JsonStore({
							fields : ["id", "lng", "lat", "vehicleNum",
									"vehicleBds", "vehicleType", "time",
									"deptname", "direct", "simcard", "speed",
									"address"],
							root : 'data',
							totalProperty : 'total'
						});
				...
				this.intorgridPanel = new Ext.grid.GridPanel({
							...
							listeners : {
								...
								rowclick : function(grid, index) {
									...
									this.fireEvent("mintorClick", targetId,
											record);
								}
							}
						});
		...
		getAlarmStore : function() {
			if (!this.alarmStore) {
				this.alarmStore = new Ext.data.JsonStore({
							fields : ["id", "lng", "lat", "vehicleNum",
									"deviceId", "vehicleType", "speed",
									"vehicleColor", "deptname", "alarmType",
									"simcard", "address", "direct"],
							root : 'data',
							totalProperty : 'total'
						});
			}
				...
				this.alarmGrid = new Ext.grid.GridPanel({
							...
							listeners : {
								...
								rowclick : function(grid, index) {
									var record = grid.getStore().getAt(index);
									var targetId = record.get("id");
									this.fireEvent("alarmClick", targetId,
											record);
								}
							}
						});
