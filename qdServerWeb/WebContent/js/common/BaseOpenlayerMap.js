(function() {
	...
	GAS.BaseMap = Ext.extend(Ext.Panel, {
		...
		initMap : function() {
			if (!this.map) {
				var point = this.createPoint(this.defaultCenter[0],
		...
		// 更新marker的位置信息【位置和图标】
		updateMarkerIconAndPostion : function(marker, icoUrl, newpoint) {
			var imageDom = document.createElement("img");
			imageDom.setAttribute("src", icoUrl);
			imageDom.style.cursor = "pointer";
			marker.setPosition(newpoint);
		...
		showDrawPolugon : function(listArr) {
			...
			if (array.length > 0) {
				...
				for (var i = 0; i < array.length; i++) {
					...
					var coorlnglat = this.createPoint(lnglatArr[0],
							lnglatArr[1]);
