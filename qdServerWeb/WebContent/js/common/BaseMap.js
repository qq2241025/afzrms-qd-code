(function() {
	...
	GAS.BaseMap = Ext.extend(Ext.Panel, {
		constructor : function(config) {
			...
			this.maxZoom = 19;
			this.defaultZoom = 16;
			this.baseLayerUrl = GAS.TitleURL["tiandituUrl"];// mapTileUrl【正式地图】
															// mapTestUrl【测试地图】
		...
		addMarker : function(cfg) {
			...
			if (cfg["clickEnable"]) {
				...
				marker.on("click", function(e) {
							var opp = L.popup().setLatLng(point)
									.setContent(text).openOn(me.map);
							me.popWin = opp;
						});
			}
			return marker;
		...
		// 创建折线
		/**
		 * cfg{ point : [] , lineOption : { color: 'blue', weight: 2, ......
		 * ...... } }
		 */
				...
				this.mapcontainer = new Ext.Container({
							...
							listeners : {
								...
								afterRender : function(panel) {
									...
									var filelist = [GAS.config.rootPath
											+ "lib/leaflet/leaflet.js"];
									GAS.LoadScript.load(filelist, this.initMap,
											this, false);
								}
							}
						});
		...
		drawGonhandler : function() {
			...
			var filelist = [
					...
					GAS.config.rootPath + "lib/fix/Draw.ToolTip.js"], me = this, map = this
					.getMap();
			// 回调函数
		...
		showDrawPolugon : function(listArr) {
			...
			if (array.length > 0) {
				...
				for (var i = 0; i < array.length; i++) {
					var lnglatStr = array[i], lnglatArr = lnglatStr.split(",");
			...
			var btn = new Ext.form.Checkbox({
						...
						listeners : {
							...
							check : function(check, flag) {
								console.log(flag);
								var showMap = flag, layer = this.getBaseLayer()
										.getContainer();;
								if (flag) {
