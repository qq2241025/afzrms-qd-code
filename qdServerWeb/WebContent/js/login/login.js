(function() {
	GAS.LOGIN = Ext.extend(Ext.Window, {
		constructor : function(config) {
			this.config = config;
			this.dologinUrl = GAS.config["globalProxy"] + "/login?_dc="
		...
		getNorthPanel : function() {
			if (!this.northPanel) {
				this.northPanel = new Ext.Panel({
						...
						// height: 146,
						// layout:"fit",
						// html: '<image src="images/DBLogins.png"
		...
		getloginForm : function() {
			if (!this.formpanel) {
				this.formpanel = new Ext.FormPanel({
					...
					items : [this.getUserUsername(), this.getPassword(),
		...
		getLoginParams : function(from) {
			if (from) {
				var data = {
					account : GAS.utils.base64encode(this.getUserUsername()
							.getValue()),
					password : GAS.utils.base64encode(this.getPassword()
							.getValue()),
		...
		submit : function() {
			...
			var callHandler = function(res, req) {
				...
				if (res["success"]) {
					...
					window.location.href = GAS.config["mainPageUrl"]
							+ "?tokenID=" + tookeId;
					return;
				} else {
